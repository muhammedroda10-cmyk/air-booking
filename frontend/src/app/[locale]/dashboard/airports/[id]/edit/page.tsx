...
export default function EditAirportPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchAirport = async () => {
        try {
            ...
            toast({ title: 'Error', description: 'Failed to fetch airport details', variant: 'destructive' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast({ title: 'Airport updated successfully' });
            router.push('/dashboard/airports');
        } catch (error: any) {
            console.error('Failed to update airport', error);
            toast({ title: 'Error', description: error.response?.data?.message || 'Failed to update airport', variant: 'destructive' });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="mb-6">
                <Link href="/dashboard/airports" className="text-sm text-muted-foreground hover:text-primary flex items-center gap-2 mb-2">
